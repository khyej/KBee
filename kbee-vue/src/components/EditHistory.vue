<template>
    <div>
        <div class="flex gap-2 mb-4">
            ...
            <button @click="mode = 'income'" :class="btnClass('income')">
                수입
            </button>
            <button @click="mode = 'expense'" :class="btnClass('expense')">
                지출
            </button>
        </div>
            ...
            >
                <div class="flex justify-between">
                    <div>
                        <p>
                            <strong
                                >{{ item.amount.toLocaleString() }}원</strong
                            >
...
const fetchData = async () => {
    const res1 = await axios.get('/api/incomes?user_id=1');
    const res2 = await axios.get('/api/expenses?user_id=1');
...
const applyEdit = async (item) => {
    ...
    if (item.type === 'expense')
        updated.payment_method = editForm.payment_method;
